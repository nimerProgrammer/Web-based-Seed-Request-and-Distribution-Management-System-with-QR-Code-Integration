$(document).ready(function () {
        ...
        $.ajax({
            ...
            success: function (response) {
                if (response.exists) {
                    ...
                    $("#seasonNameFeedback")
                        .text(
                            "This season name already exists with the selected year."
                        )
                        .show();
                    $("#seasonYearFeedback")
                        .text(
                            "This season year is already taken for the selected name."
                        )
                        .show();
            ...
            error: function () {
                console.error(
                    "AJAX error: Could not check season combination."
                );
            },
        });
    }
    ...
    $("#season_name, #season_year").on(
        "input change",
        validateSeasonCombination
    );
        ...
        $.ajax({
            ...
            success: function (response) {
                if (response.conflict) {
                    ...
                    $("#startDateFeedback")
                        .text(
                            "Start date overlaps with an existing cropping season."
                        )
                        .show();
                } else {
                    $("#season_start_date").removeClass("is-invalid");
    ...
    function validateEditSeasonCombination() {
        ...
        $.ajax({
            ...
            success: function (response) {
                if (response.exists) {
                    $("#edit_season_name, #edit_season_year").addClass(
                        "is-invalid"
                    );
                    ...
                    $("#edit_season_name, #edit_season_year").removeClass(
                        "is-invalid"
                    );
            ...
            error: function () {
                console.error(
                    "AJAX error: Could not check edit season combination."
                );
            },
        });
    }
    ...
    function validateEditStartYearMatch() {
        ...
        if (!selectedYear || !startDate) {
            $("#edit_season_year, #edit_season_start_date").removeClass(
                "is-invalid"
            );
            $("#editStartDateFeedback, #editSeasonYearFeedback")
                .text("")
                .hide();
            return;
        }
        ...
        const startYear = new Date(startDate).getFullYear();
        if (parseInt(selectedYear) !== startYear) {
            $("#edit_season_year, #edit_season_start_date").addClass(
                "is-invalid"
            );
    ...
    $("#edit_season_start_date, #edit_season_end_date").on(
        "input",
        function () {
            const startDate = $("#edit_season_start_date").val();
            ...
            if (startDate && endDate) {
                if (endDate <= startDate) {
                    $(
                        "#edit_season_start_date, #edit_season_end_date"
                    ).addClass("is-invalid");
                    $("#editStartDateFeedback")
                        .text("Start date must be before end date.")
                        .show();
                    ...
                    $("#editStartDateFeedback, #editEndDateFeedback")
                        .text("")
                        .hide();
                }
            } else {
